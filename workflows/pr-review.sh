...
add_check 4 'Is each piece of essential state stored in one clear source of truth?'
add_check 4 'Are values that can be calculated from existing state derived instead of stored and manually synchronized?'
add_check 4 'Do explicit states and transitions represent meaningful domain rules, rather than duplicate information that can be derived?'
add_check 4 'Are expensive derivations handled efficiently without introducing redundant mutable state?'

add_section '6. Tests and evidence'
add_check 5 'Do meaningful tests prove the changed behavior and important regressions, including edge cases and failure paths?'
add_check 5 'Is the evidence at the right level, with related fixtures, scripts, supported implementations, or manual inspection covered where needed?'

add_section '7. Customer-visible behavior'
add_check 6 'Are customer-facing workflows, content, errors, and empty states understandable, with visible changes communicated where needed?'
add_check 6 'Are effects on customer data, latency, availability, and user expectations understood and acceptable?'
...
add_section '9. CX and support readiness'
add_check 8 'Do CX and support have the behavior, known limitations, and troubleshooting guidance they need?'
add_check 8 'Can support diagnose likely failures safely, with training or escalation paths updated where needed?'

add_section '10. Future maintainability'
add_check 9 'Does the change use built-in framework or library mechanisms where they fit, instead of reimplementing them by hand?'
add_check 9 'Can the implementation be simplified without losing needed behavior or clarity?'
add_check 9 'Are names and domain terms clear, consistent, and aligned with the concepts the code represents?'

add_section '11. Need, intent, and scope'
add_check 10 'Is this change needed, and can I explain the need it addresses and how the PR connects the diff to that requirement or decision?'
add_check 10 'Does the change deliver the intended outcome with a focused scope that fits the product direction?'

add_section '12. Design and contracts'
add_check 11 'Are responsibilities, files, dependencies, and reused capabilities in the right system boundaries?'
add_check 11 'Is the complexity justified, are existing contracts preserved, and is feedback about material issues rather than preference?'

add_section '13. Review feedback and follow-up'
add_check 12 'Are comments concrete, prioritized, and actionable, with blockers clearly separated from optional suggestions?'
add_check 12 'Have I re-reviewed material amendments, closed outstanding concerns, and acknowledged useful decisions?'
