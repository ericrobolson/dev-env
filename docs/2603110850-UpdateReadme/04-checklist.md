...
- [ ] Confirm `cursor-agent` installation method with user (currently **UNKNOWN**)
- [ ] Decide whether to document `$MODEL` bug or fix it first

---

## Step 1: Header & Introduction

- [ ] Replace existing title and intro paragraph
- [ ] Add "Core Tools" table with `one-shot` and `build-feature`

---

## Step 2: Prerequisites Section

- [ ] Add "## Prerequisites" heading after intro
- [ ] Add "### Required" subsection with dependency table
  - [ ] Document `cursor` installation
  - [ ] Document `cursor-agent` installation (pending clarification)
  - [ ] Document `claude` CLI installation
- [ ] Add "### Environment Variables" subsection
  - [ ] Document `ANTHROPIC_API_KEY`
- [ ] Add "### Platform Support" subsection
  - [ ] Document macOS support
  - [ ] Document Linux limitations
  - [ ] Document Windows status

---

## Step 3: Installation Section

- [ ] Add clone command with repo URL placeholder
- [ ] Add shell function definitions for `~/.zshrc` or `~/.bashrc`
  - [ ] `build-feature()` function
  - [ ] `one-shot()` function
- [ ] Add shell reload command

---

## Step 4: one-shot Usage Section

- [ ] Add "## Usage: one-shot" heading
- [ ] Add syntax section: `one-shot <feature-name> <prompt>`
- [ ] Add working example
...
- [ ] Document known `$MODEL` bug

---

## Step 5: build-feature Usage Section

- [ ] Add "## Usage: build-feature" heading
- [ ] Add syntax section: `build-feature <feature-name> <doc-directory> <prompt>`
- [ ] Add working example
- [ ] Add pipeline diagram (ASCII art)
- [ ] Add stages table with output files and models
- [ ] Document output directory structure
- [ ] Document interactive flow (cursor agent mode)
- [ ] Add exit codes table
...
- [ ] Document `AGENT_TYPE` setting
  - [ ] Location in `bin/build-feature`
  - [ ] Available values: `cursor`, `claude`
- [ ] Document model configuration
  - [ ] Planning stages model
  - [ ] Implementation stage model
  - [ ] Note that models are not yet configurable
...
- [ ] Document "cursor-agent: command not found" error and workaround
- [ ] Document "claude: command not found" error and fix
- [ ] Document audio notification issues on non-macOS
- [ ] Document partial file cleanup after agent failure
...
- [ ] Run `one-shot` with documented example — verify output matches docs
- [ ] Run `build-feature` with documented example — verify output matches docs
- [ ] Verify all file paths in examples are accurate
- [ ] Test shell function installation steps in clean shell
...
| # | Question | Status |
|---|----------|--------|
| 1 | How is `cursor-agent` installed? | Left to user to install |
| 2 | How does `cursor-agent` authenticate? | Left to user to authenticate |
| 3 | Should `$MODEL` bug in `one-shot` be fixed or just documented? | I fixed it in the codebase. |
| 4 | Are `opus-4.5-thinking` and `kimi-k2.5` publicly available models? | Yes. Add a comment saying it should be abstracted away in future tasks. |
